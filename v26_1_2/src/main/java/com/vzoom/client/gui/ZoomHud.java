...
import com.vzoom.client.VZoomClient;
...
/**
 * The vZoom HUD (MC 26.2 target), implemented as a layered {@link HudElement}
 * using the {@code extractRenderState} GUI model.
 *
 * <p>Responsibilities:
 * <ul>
 *   <li>drive the per-frame activation + easing of the zoom engine,</li>
 *   <li>paint the configurable zoom indicator in the chosen {@link HudStyle}
 *       with optional info lines (target, coords, fps, facing).</li>
 * </ul>
 */
public final class ZoomHud implements HudElement {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
        ...
        s.updateActivation(VZoomClient.isZoomHeld(), VZoomClient.isStateZoomable());
        boolean isActive = s.isActive();
        s.update();

        Minecraft mc = Minecraft.getInstance();
        if (mc.options != null) {
            mc.options.smoothCamera = isActive && s.config.cinematicCamera;
        }
        ...
        renderIndicator(graphics, mc, s, screenW, screenH);
    }

    // ------------------------------------------------------------------
    // Indicator (the configurable HUD readout)
    // ------------------------------------------------------------------
    private void renderIndicator(GuiGraphicsExtractor g, Minecraft mc, ZoomState s, int screenW, int screenH) {
        Font font = mc.font;
        ...
        // Raycast once and share between the coordinates and target lines.
        HitResult hit = null;
        ...
        switch (s.config.hudAlign) {
            ...
            case TOP_CENTER:
                x = (screenW - sw) / 2; y = MARGIN; break;
            case TOP_RIGHT:
                x = screenW - sw - MARGIN; y = MARGIN; break;
            case BOTTOM_LEFT:
                x = MARGIN; y = screenH - sh - MARGIN; break;
            case BOTTOM_CENTER:
                x = (screenW - sw) / 2; y = screenH - sh - MARGIN; break;
            case BOTTOM_RIGHT:
                x = screenW - sw - MARGIN; y = screenH - sh - MARGIN; break;
            case CENTER:
                x = (screenW - sw) / 2; y = (screenH - sh) / 2; break;
        }
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            g.text(font, Component.literal(lines.get(i)), PAD, PAD + i * font.lineHeight, color, true);
        }
    ...
    private static HitResult computeHit(Minecraft mc, ZoomState s) {
        ...
        double dist = Math.min(256.0, 6.0 + s.getCurrentZoom() * 5.0);
        return cam.pick(dist, 0.0F, false);
    }

    /** Coordinates of the block/entity the camera is currently aimed at. */
    ...
    private static String targetLine(Minecraft mc, HitResult hit) {
        ...
        if (hit.getType() == HitResult.Type.ENTITY && hit instanceof EntityHitResult) {
            ...
            return e.getDisplayName().getString() + " (" + fmtDist(d) + ")";
        } else if (hit.getType() == HitResult.Type.BLOCK && hit instanceof BlockHitResult) {
