...
/**
 * ModMenu-integrated settings screen for MC 26.x. Built entirely from the
 * vanilla Button API (no external config library) and the new
 * {@code extractRenderState} GUI model, so it stays lightweight and portable.
 *
...
public class VZoomConfigScreen extends Screen {
    ...
    private void header(String key) {
        // Section headers are plain text (drawn in extractRenderState), not buttons.
