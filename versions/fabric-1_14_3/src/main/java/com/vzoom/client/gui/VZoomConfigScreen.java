...
public class VZoomConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        toggleRow("vzoom.config.cinematic", () -> cfg.cinematicCamera, v -> { cfg.cinematicCamera = v; applyCinematic(cfg); children.clear(); });
        ...
        addButton(new Button(cx - BTN_W - 4, bottomY, BTN_W, BTN_H, translate("vzoom.config.reset"),  b -> {
        ...
        addButton(new Button(cx + 4, bottomY, BTN_W, BTN_H, translate("gui.done"),  b -> onClose()));
    }
    ...
    private void stepperRow(String key, Supplier<Double> getter, Consumer<Double> setter,
        ...
        if (inBand(sy)) {
            ...
            addButton(valueBtn);
            addButton(new Button(cx - VALUE_W / 2 - SIDE_W - 4, sy, SIDE_W, BTN_H, String.valueOf("-"),  b -> {
                double v = clamp(getter.get() - step, min, max);
                setter.accept(v);
                valueBtn.setMessage(valueLabel(key, v));
                VZoomConfigIo.save();
            }));
            addButton(new Button(cx + VALUE_W / 2 + 4, sy, SIDE_W, BTN_H, String.valueOf("+"),  b -> {
                double v = clamp(getter.get() + step, min, max);
                setter.accept(v);
                valueBtn.setMessage(valueLabel(key, v));
                VZoomConfigIo.save();
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scroll) {
        if (maxScroll > 0) {
            ...
            if (scrollOffset != old) children.clear();
        }
    ...
    public void render(int mouseX, int mouseY, float partialTick) {
        net.minecraft.client.gui.GuiComponent.fill(0, 0, this.width, this.height, 0xC0101010);
        super.render(mouseX, mouseY, partialTick);

        drawString(this.font, this.title.getString(), (this.width - this.font.width(this.title.getString())) / 2, 14, 0xFFFFFFFF);
        ...
        for (Header h : headers) {
            ...
            if (y + this.font.lineHeight > visTop && y < visBottom) {
                ...
                drawString(this.font, h.text, tx, ty, 0xFFFFD27A);
            }
        }
        ...
        if (maxScroll > 0) {
            ...
            int sx = this.width - 6;
            net.minecraft.client.gui.GuiComponent.fill(sx - 1, visTop, sx + 1, visBottom, 0x44FFFFFF);
            net.minecraft.client.gui.GuiComponent.fill(sx - 2, thumbY, sx + 2, thumbY + thumbH, 0xAAFFFFFF);
        }
    }
    ...
    private static String toggleLabel(String key, boolean on) {
        return String.valueOf(translate(key) + ": " + (on ? "ON" : "OFF"));
    }

    private static <T extends Enum<T>> String cycleLabel(String key, T value) {
        return String.valueOf(translate(key) + ": " + prettify(value.name()));
    }

    private static String valueLabel(String key, double value) {
        String shown;
        ...
        return String.valueOf(translate(key) + ": " + shown);
    }

    private static String intLabel(String key, int value) {
        return String.valueOf(translate(key) + ": " + value);
    }

    private static String colorLabel(String key, int color) {
        return String.valueOf(translate(key));
    }
