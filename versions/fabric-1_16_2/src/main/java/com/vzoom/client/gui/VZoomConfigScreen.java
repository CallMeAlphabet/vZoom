...
public class VZoomConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        addButton(new Button(cx - BTN_W - 4, bottomY, BTN_W, BTN_H, new TranslatableComponent("vzoom.config.reset"),  b -> {
        ...
        addButton(new Button(cx + 4, bottomY, BTN_W, BTN_H, new TranslatableComponent("gui.done"),  b -> onClose()));
    }
    ...
    private void stepperRow(String key, Supplier<Double> getter, Consumer<Double> setter,
        ...
        if (inBand(sy)) {
            ...
            addButton(valueBtn);
            addButton(new Button(cx - VALUE_W / 2 - SIDE_W - 4, sy, SIDE_W, BTN_H, new TextComponent("-"),  b -> {
                double v = clamp(getter.get() - step, min, max);
                setter.accept(v);
                valueBtn.setMessage(valueLabel(key, v));
                VZoomConfigIo.save();
            }));
            addButton(new Button(cx + VALUE_W / 2 + 4, sy, SIDE_W, BTN_H, new TextComponent("+"),  b -> {
                double v = clamp(getter.get() + step, min, max);
                setter.accept(v);
                valueBtn.setMessage(valueLabel(key, v));
                VZoomConfigIo.save();
