...
public class VZoomConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        addRenderableWidget(new Button(cx - BTN_W - 4, bottomY, BTN_W, BTN_H, Component.translatable("vzoom.config.reset"),  b -> {
        ...
        addRenderableWidget(new Button(cx + 4, bottomY, BTN_W, BTN_H, Component.translatable("gui.done"),  b -> onClose()));
    }
    ...
    private void stepperRow(String key, Supplier<Double> getter, Consumer<Double> setter,
        ...
        if (inBand(sy)) {
            ...
            addRenderableWidget(new Button(cx - VALUE_W / 2 - SIDE_W - 4, sy, SIDE_W, BTN_H, Component.literal("-"),  b -> {
                double v = clamp(getter.get() - step, min, max);
                setter.accept(v);
                valueBtn.setMessage(valueLabel(key, v));
                VZoomConfigIo.save();
            }));
            addRenderableWidget(new Button(cx + VALUE_W / 2 + 4, sy, SIDE_W, BTN_H, Component.literal("+"),  b -> {
                double v = clamp(getter.get() + step, min, max);
                setter.accept(v);
                valueBtn.setMessage(valueLabel(key, v));
                VZoomConfigIo.save();
